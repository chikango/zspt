	...
	var init = function(jQuery) {
		...
		function migrateWarnProp(obj, prop, value, msg) {
			if (Object.defineProperty) {
				...
				try {
					...
					// IE8 is a dope about Object.defineProperty, can't warn
					// there
				}
			}
		...
		var attrFn = jQuery("<input/>", {
			size : 1
				...
				&& jQuery.attrHooks.value.set || function() {
					return undefined;
		...
		jQuery.attr = function(elem, name, value, pass) {
			...
			if (pass) {
				...
				if (elem
						...
						&& (attrFn ? name in attrFn : jQuery
								.isFunction(jQuery.fn[name]))) {
			...
			if (!jQuery.attrHooks[lowerName] && rboolean.test(lowerName)) {
				jQuery.attrHooks[lowerName] = {
					get : function(elem, name) {
						...
						// properties
						// Fall back to attribute presence where some booleans
						// are not supported
					...
					set : function(elem, value, name) {
						...
						if (value === false) {
							...
							// value is true since we know at this point it's
							// type boolean and not false
							// Set boolean attributes to the same name and set
							// the DOM property
		...
		var matched, browser, oldInit = jQuery.fn.init, oldParseJSON = jQuery.parseJSON,
		...
		jQuery.fn.init = function(selector, context, rootjQuery) {
			...
			if (selector && typeof selector === "string"
				...
				// (#9521)
				// Note that this may break jQuery 1.6.x code that otherwise
				// would work.
				if (match[0].charAt(0) === "#") {
				...
				if (jQuery.parseHTML) {
					return oldInit.call(this, jQuery.parseHTML(match[2],
							context, true), context, rootjQuery);
				}
			}
		...
		var rscriptType = /\/(java|ecma)script/i, oldSelf = jQuery.fn.andSelf
				|| jQuery.fn.addBack;
					...
					for (i = 0; (elem = ret[i]) != null; i++) {
						...
						if (!(jQuery.nodeName(elem, "script") && handleScript(elem))) {
							...
							if (typeof elem.getElementsByTagName !== "undefined") {
								...
								// former ancestor and advance our index beyond
								// them
